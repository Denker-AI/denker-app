...
def main():
    """Entry point for the Document Loader MCP server script."""
    logger.info("Starting Document Loader MCP server via main()")
    try:
        app.run()
    except Exception as e:
        logger.error(f"Error starting Document Loader server: {e}")
