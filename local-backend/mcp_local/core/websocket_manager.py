...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, query_id: str, conversation_id: Optional[str] = None):
        ...
        async with self._lock:
            try:
                ...
                self.logger.info(f"🔌 WebSocket CONNECTED for query_id: {query_id}, conversation_id: {conversation_id}. Total active: {len(self.active_connections)}")
    ...
    async def disconnect(self, query_id: str):
        ...
        if query_id in self.active_connections:
            ...
            if self.connection_states.get(query_id) == CONNECTION_STATE_CLOSING or self.connection_states.get(query_id) == CONNECTION_STATE_CLOSED:
                ...
                self.logger.info(f"🔌 WebSocket DISCONNECTED for query_id: {query_id}. Total active: {len(self.active_connections)}")
                return
    ...
    async def send_consolidated_update(
        ...
        """
        Send a consolidated update message to a specific client via WebSocket.
        
        Args:
            query_id: The query ID of the client to send the message to.
            update_type: The category of the update (e.g., 'status', 'progress', 'result', 'plan').
        ...
        """
        # Log the consolidated update request
        if workflow_type:
        ...
        # DISABLED: Streaming for plan messages to prevent duplicates
        # Streaming was causing duplicate "Plan Generated" and "Plan Completed" messages
        # if update_type == "plan":
        #     self.logger.info(f"📤 Using streaming for plan message to {query_id}")
        ...
        #         stream_delay=0.03  # Slightly faster for plans (30ms between words)
        #     )
        #     return
    ...
    async def send_streaming_update(
        ...
        """
        Send a streaming update message word by word to a specific client via WebSocket.
        
        Args:
            query_id: The query ID of the client to send the message to.
            update_type: The category of the update (e.g., 'result').
            message: The complete message to stream word by word.
            data: Optional dictionary containing additional structured data.
            workflow_type: Optional string indicating the workflow ('router', 'orchestrator', etc.)
            stream_delay: Delay in seconds between each word (default: 0.05s = 50ms)
        """
        # Log the streaming request
        if workflow_type:
            self.logger.debug(f"[{query_id}] send_streaming_update called with workflow_type: {workflow_type}")
        else:
            self.logger.debug(f"[{query_id}] send_streaming_update called without specific workflow_type.")
            
        self.logger.info(f"📤 Starting streaming update to {query_id} (Type: {update_type}, Workflow: {workflow_type or 'N/A'}) - Message length: {len(message)} chars")
        ...
        if total_words == 0:
            # If no words, send empty message as final
            full_payload = {
                "update_type": update_type,
                "queryId": query_id,
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "message": "",
                "data": {**(data if data is not None else {}), "is_streaming": True, "is_final": True},
                "streaming": {
                    "is_streaming": True,
                    "is_final": True,
                    "word_index": 0,
        ...
        for i, word in enumerate(words):
            # Check if connection is still active before sending each word
            if not self.is_connected(query_id):
                self.logger.warning(f"⚠️ Connection lost during streaming for {query_id}, stopping at word {i+1}/{total_words}")
                break
                
            partial_message = ' '.join(words[:i+1])
            ...
            full_payload = {
                ...
                "data": {**(data if data is not None else {}), "is_streaming": True, "is_final": is_final},
                "streaming": {
                    "is_streaming": True,
                    "is_final": is_final,
                    "word_index": i,
            ...
            # Add delay between words (except for the final word)
            if not is_final:
                await asyncio.sleep(stream_delay)
        
        self.logger.info(f"📤 Completed streaming update to {query_id} - Sent {total_words} words")
    ...
    async def handle_client_message(self, query_id: str, message_data: str):
        ...
        try:
            ...
            # Handle human input response
                ...
                if conversation_id:
                    # Persistence of human input is now handled by the backend via API.
