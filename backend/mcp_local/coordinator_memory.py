...
class CoordinatorMemory:
    ...
    async def initialize(self):
        """Initialize the memory system and load the knowledge graph."""
        try:
            ...
            # Load the existing graph or create a new one if it doesn't exist
            self.graph = await self.memory_tools.mcp_memory_read_graph(random_string="init")
            logger.info("Memory graph loaded successfully")
            
            # Ensure we have the core entity types
